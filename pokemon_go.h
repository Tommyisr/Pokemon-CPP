...
class PokemonGo {
	const World* world;
	std::map<std::string, Trainer*> allTrainers;
public:
	...
	PokemonGo& operator=(const PokemonGo&) = delete;
	// A destructor.
	~PokemonGo() = default;
	...
	//        not exist.
	void AddTrainer(const std::string& name, const Team& team,
	...
	void MoveTrainer(std::string trainer_name, int dir);
