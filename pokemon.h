...
private:
	...
	int level;
	double cp;
	set<PokemonType> types; //eNum
	/*
	 * this function returns the HitPower of the pokemon it is operated on
	 * HitPower = cp*level
	 */
	double getHitPower() const;
	/*
	 * this function gets a new hp and changes the current hp to the new one
	 */
	void setHP(const double newHP);
//	creates a string out of a pokemon's set of types
	std::string createAStringOutOfTypes()const;
//	transfer a single int to a PokemonType string
	std::string getPokemonTypeString(int i)const;
...
public:
	...
	Pokemon(const std::string& species, const std::set<PokemonType>& types,
	...
	friend std::ostream& operator<<(std::ostream& output,
		...
		output << pokemon.species << "(" << pokemon.level << "/" << pokemon.cp
				<< "/" << pokemon.hp << ")"<<pokemon.createAStringOutOfTypes();

		//TODO/////////////

//		std::set<PokemonType>::iterator it;
//		for (it = pokemon.types.begin(); it != pokemon.types.end(); ++it) {
//			output << *it;
//		}
	...
	// @return true if the hitten Pokemon died (HP reached zero).
